...
function remove_default_post_type()
{
	...
	} elseif (current_user_can('phcns_hs')) {
...
?>
	...
	<?php } elseif (current_user_can('phcns_hs')) {
	?>
		<style>
			/* Ví dụ: ẩn ACF Option Page cụ thể */
...
add_action('admin_footer', function () {
	if (current_user_can('phcns_hs') && isset($_GET['page']) && $_GET['page'] === 'cfdb7-list.php') {
	?>
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				const allowedFid = '660';
				...
				rows.forEach(row => {
					...
					if (link) {
						...
						if (!fidMatch || fidMatch[1] !== allowedFid) {
							row.remove(); // Ẩn dòng không đúng form ID
						}
					}
				});
			});
...
add_action('admin_init', function () {
	if (
		current_user_can('phcns_hs') &&
		isset($_GET['page']) && $_GET['page'] === 'cfdb7-list.php' &&
		isset($_GET['fid']) && $_GET['fid'] !== '660'
	) {
		wp_die('Bạn không có quyền truy cập form này.');
	}
});
