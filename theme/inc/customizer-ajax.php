...
function filter_details_symbol() {
					...
					foreach ( $response_GetForeignInvestors->d as $GetForeignInvestors )
					{
						...
						if ( $i_GetForeignInvestors < 8 )
						{
							...
							<li class="flex items-center [&:nth-child(odd)]:bg-white bg-[#EBF4FA]">
								...
								<div class="flex-1 px-3 py-2 min-h-10 flex items-center justify-center">
									<?php
		...
		{ ?>
			<div
				...
				<div class="relative text-center">
					...
					<?php } ?>
					<svg id="progress-ring" class="mx-auto" width="166" height="166" viewBox="0 0 166 167"
						fill="none" xmlns="http://www.w3.org/2000/svg">

						<circle cx="83.0342" cy="83.6479" r="72.3521" stroke="#295CA9" stroke-width="21"
							stroke-linecap="round" stroke-linejoin="round" />

						<circle id="progress-circle" cx="83.0342" cy="83.6479" r="72.3521" stroke="#F2B122"
	...
	} elseif ( $type_form == 'sg_ttvmcp' )
	{
		...
		{ ?>
			...
			foreach ( $response_GetNews->d as $news )
			{
				...
				?>
				<div class="news_service-item document_item-popup" data-newstype="1"
					data-modal-target="document-modal" data-modal-toggle="document-modal"
					data-doccument="<?php echo $news->attachedfileurl ?>" data-id="<?php echo $news->newsid ?>">
	...
	} elseif ( $type_form == 'securityBasicInfo-symbol' )
	{
		...
		if ( $data )
		{
			...
			$minValue = floor( $minValue / 10 ) * 10;
			?>
			<div id="chart" data-height="514" data-fromdate="<?php echo $fromdate ?>"
				data-time_cache="<?php echo $time_cache ?>" data-maxvalue="<?php echo $maxValue; ?>"
				data-minvalue="<?php echo $minValue; ?>" data-stock='<?php echo $stocksDataJson ?>'></div>
			<?php
		}
			...
			if ( $response_list_bsc )
			{
				...
				<div
					...
					foreach ( $response_list_bsc->d as $list_bsc )
					{
						...
						{
							?>
							<div class="flex items-center <?php echo $i % 2 == 0 ? '' : 'bg-[#EBF4FA]' ?> <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? '' : 'gap-4' ?> bsc_need_crawl_price"
								...
								<div class="flex-1 min-w-[110px] <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'min-h-[60px] py-1' : 'py-[12px]' ?> flex items-center justify-center leading-[1.125] px-3 font-bold bsc_need_crawl_price-text_color-closePrice"
