...
<main>
    ...
    <section class="bg-gradient-blue-to-bottom-50  <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'pt-12 pb-[130px]':'py-[50px]' ?>">
        <div class="container">
                        ...
                        <div class="sticky top-5 z-10">
                            ...
                            ?>
                                ...
                                <?php else : ?>
                                    ...
                                    <ul class="overflow-y-auto absolute py-2 z-30 w-full max-h-64 scroll-bar-custom block [&:not(.active)]:opacity-0 opacity-100 [&:not(.active)]:pointer-events-none transition-all duration-500 origin-top-left scale-x-100 [&:not(.active)]:scale-y-0 scale-100 bg-[#F3FBFE] p-2 prose-a:block rounded text-xs">
                                        <?php
                                        $current_term_id = get_queried_object_id();
                                        foreach ($terms as $term) :
                                            $active_class = ($current_term_id === $term->term_id) ? 'active' : '';
                    ...
                    <?php if (get_field('type_danh_muc', get_queried_object()) == 'default') { ?>
                        <?php if (have_posts()) : ?>
                            <div class="list__news">
                                <div class="grid <?php echo !wp_is_mobile() && !bsc_is_mobile() ?' grid-cols-2 gap-x-6 gap-y-8':'grid-cols-1 gap-y-4' ?>">
                                    <?php
                                    while (have_posts()) :
                                        the_post();
                                    ?>
                                        <div class="<?php echo !wp_is_mobile() && !bsc_is_mobile() ?'flex flex-col':'grid grid-cols-2 gap-4 items-center space-y-0' ?>">
                                            <?php
