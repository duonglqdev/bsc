				...
				<div class="<?php echo !wp_is_mobile() && !bsc_is_mobile() ? 'w-[320px] max-w-[35%]' : 'w-full' ?>">
					<div class="bg-[#F5FCFF] py-5 px-6 rounded-[10px]">
						...
						<?php if (!wp_is_mobile() && !bsc_is_mobile()) { ?>
							...
							<div class="mt-[12px] <?php if (get_field('check_tuyen_xong') || $deadline_date < $current_date) echo 'opacity-50' ?>">
								<button data-modal-target="popup-modal" data-modal-toggle="popup-modal"
									class="btn-base-yellow w-full h-full rounded-xl" <?php if (get_field('check_tuyen_xong') || $deadline_date < $current_date) echo 'disabled' ?>>
