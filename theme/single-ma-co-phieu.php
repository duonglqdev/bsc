<?php if ( $args['data'] ) { $news=$args['data']; $symbol=strtoupper( $args['symbol'] ); $first_symbol=substr( $symbol,
	...
	); if ( get_field( 'cdttcp1_background_banner' , 'option' ) || get_field( 'cdttcp1_background_banner_mobile'
	, 'option' ) ) { $banner=wp_get_attachment_image_url( wp_is_mobile() && bsc_is_mobile() &&
	get_field( 'cdttcp1_background_banner_mobile ' , 'option' ) ? get_field( 'cdttcp1_background_banner_mobile '
	, 'option' ) : get_field( 'cdttcp1_background_banner ' , 'option' ), 'full' ); }
	$style=get_field( 'cdttcp1_background_banner_display' , 'option' ) ?: 'default' ;
	$title_breadcrumb=get_field( 'cdttcp1_title' , 'option' ); $breadcrumb='cophieu' ; } else { wp_redirect(
	home_url( '/404' ), 301 ); exit; } $check_logout=bsc_is_user_logged_out(); $class=$check_logout['class'] ?? '' ;
	get_header(); ?>
	<main>
