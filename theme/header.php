					...
					<form id="form-shares" action="<?php echo get_home_url() ?>"
						...
						<div class="relative pl-3 pr-6">
							<input type="text" name="s" id="search-shares"
								...
								autocomplete="off">
							<ul
								class="shares-result absolute py-2 rounded-md z-10 w-full h-52 overflow-y-auto pr-3 scroll-bar-custom block [&:not(.active)]:opacity-0 opacity-100 [&:not(.active)]:pointer-events-none transition-all duration-500 origin-top-left scale-x-100 [&:not(.active)]:scale-y-0 scale-100 bg-gradient-blue text-white prose-a:block prose-a:px-3 prose-a:py-2 prose-a:font-medium prose-a:font-Helvetica">
								<li class="no-results text-center p-4 font-bold hidden"><?php _e('Không thấy kết quả!', 'bsc') ?></li>
							</ul>
						</div>
					</form>
					<ul
