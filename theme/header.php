			...
			<div class="bg-gradient-blue py-2 text-white relative lg:block hidden">
				...
				<div class="container">
					<div class="lg:flex items-center justify-end">
						<div class="bg-[#A3C3FB80] rounded-[10px] p-[2px]">
							<form id="form-shares" action="<?php echo get_home_url() ?>"
								class="border border-[#739EDD] border-opacity-60 rounded-lg flex items-center bg-[#1E4285] text-xs leading-none">
								<div class="relative">
									<i class="absolute top-1/2 -translate-y-1/2 left-4 size-4 shrink-0">
										<?php echo svg('search-white', '16', '16') ?>
									</i>
									<i class="absolute top-1/2 -translate-y-1/2 right-4 size-4 shrink-0 cursor-pointer">
										<?php echo svg('close-circle', '16', '16') ?>
									</i>
									<input type="text" name="s" id="search-shares"
										placeholder="<?php _e('Tra cứu', 'bsc') ?>..."
										class="bg-transparent px-10 py-1 border-none focus:outline-0 focus:ring-transparent pb-2 font-medium min-w-[560px] h-9 peer text-white placeholder:text-white focus:shadow-none focus:border-none placeholder:opacity-60 hover:placeholder:opacity-100 placeholder:transition-all placeholder:duration-700 max-w-[167px]"
										autocomplete="off">
									<div
										class="shares-result text-black absolute top-[108%] z-30 w-full overflow-y-auto scroll-bar-custom block [&:not(.active)]:opacity-0 opacity-100 [&:not(.active)]:pointer-events-none transition-all duration-500 origin-top-left scale-x-100 [&:not(.active)]:scale-y-0 scale-100 bg-white rounded-[4px]">
										<div class="tab-wrapper w-full">
											<div
												class="tab-button px-4 w-full border-b border-[#C9CCD2] flex items-end gap-6 font-semibold text-[14px] leading-[1.2]">
												<div
													class="py-2.5 px-[6px] w-fit border-b-2 [&:not(.active,:hover)]:border-transparent border-primary-400 cursor-pointer transition-colors duration-300">
													<?php _e('Cổ phiếu', 'bsc') ?>
												</div>
												<div
													class="py-2.5 px-[6px] w-fit border-b-2 [&:not(.active,:hover)]:border-transparent border-primary-400 cursor-pointer transition-colors duration-300">
											...
											<div class="tab-content">
												<div class="py-2.5 space-y-2">
													<?php for ($i = 0; $i < 5; $i++): ?>
														<div
															class="group py-1 px-4 flex items-center justify-between hover:bg-[#EBF4FA] cursor-pointer transition-colors duration-300">
															<div class="flex items-center gap-[9px]">
																<span
																	class="font-semibold text-base group-hover:text-[#FFB81C] transition-colors duration-300">TPB</span>
																<span
																	class="text-base text-[#31333F4D] group-hover:text-[#FFB81C] transition-colors duration-300">•</span>
																<span
																	class="font-semibold text-base group-hover:text-[#FFB81C] transition-colors duration-300">TPBank</span>
															</div>
															<div class="flex items-center gap-2 text-[14px] leading-[22px]">
																<span class="text-[#31333F99]">Mức sinh lời</span>
																<div
																	class="py-1 px-4 rounded-md overflow-hidden bg-[#1CCD83] text-white font-semibold">
																	+11%
																</div>
															</div>
														</div>
												...
												<div class="py-2.5 space-y-2 hidden">
													<?php for ($i = 0; $i < 5; $i++): ?>
														<div
															class="py-1 px-4 space-y-1 hover:bg-[#EBF4FA] transition-colors duration-300 cursor-pointer">
															<div class="text-[14px] leading-[20px] font-semibold text-green">
																Báo cáo doanh nghiệp
															</div>
															<div class="text-base line-clamp-2 font-medium">
																202401111_Cập nhật KQKD Nhóm ngành Vật liệu xây dựng, Bất động
																sản Q3/2024_<span class="font-bold text-primary-400">BSC</span>
																Research
															</div>
															<div class="text-[#31333F99] text-[14px] leading-[22px]">
																15/05/2025
															</div>
														</div>
													<?php endfor ?>
													...
													<?php for ($i = 0; $i < 5; $i++): ?>
														<div
															...
															<div class="text-base line-clamp-2">
																ĐHĐCĐ Chứng khoán BIDV (<span
																	class="font-bold text-primary-400">BSC</span>): Lợi nhuận
																mục tiêu 560 tỷ đồng, dự kiến trả cổ tức tỷ lệ 10%
															</div>
														</div>
													<?php endfor ?>
												</div>
										...
										<div
											class="py-2 px-4 bg-[#C9CCD299] flex items-center justify-end gap-1 font-medium">
											<div class="py-[2px] px-2 rounded-[4px] text-[14px] leading-[1.5] bg-[#C9CCD2]">
												<?php _e('Tab', 'bsc') ?>
											</div>
											<span class="text-[#31333F80]">
												<?php _e('Chuyển nhanh', 'bsc') ?>
											</span>
										</div>
									</div>
								</div>
				...
				<div class="flex justify-between items-center gap-3">
					<div class="flex items-center sm:gap-5 gap-3 relative lg:hidden">
						...
						<form action="<?php echo get_home_url() ?>"
							...
							<div class="relative">
								<input id="search-shares" type="text" name="s" placeholder="Tra cứu..."
					...
					if ($custom_logo_id) {
						...
						<a class="block <?php echo !wp_is_mobile() && !bsc_is_mobile() ? 'lg:static absolute left-1/2 lg:translate-x-0 -translate-x-1/2 top-1/2 lg:translate-y-0 -translate-y-1/2' : 'absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2' ?>"
							href="<?php echo get_bloginfo('url'); ?>" title="<?php echo get_bloginfo('description'); ?>">
					...
					<div
						...
						<ul
							...
							<div class="lg:hidden flex items-center justify-between mb-6">
								...
								<div class="flex items-center gap-[12px]">
									<button data-dropdown-toggle="dropdownLanguage"
										class="text-black flex items-center gap-1.5 uppercase" type="button">
										<?php echo svg('global', '24', '24') ?>
										<?php echo svg('down') ?>
									</button>
							...
							<?php if (!wp_is_mobile() && !bsc_is_mobile()) { ?>
								<div
									class="absolute xl:hidden md:hidden lg:flex top-4 right-3 w-6 h-6 rounded-full justify-center items-center bg-primary-300 z-10 close-menu">
						...
						<?php if (!wp_is_mobile() && !bsc_is_mobile()): ?>
							<?php if (have_rows('h1_button', 'option')): ?>
								<div class="flex items-center xl:gap-x-4 gap-x-3 2xl:ml-[60px] xl:ml-8 ml-3 whitespace-nowrap">
									<?php
									$i = 1;
									while (have_rows('h1_button', 'option')):
										the_row();
