...
<main>
	...
	<section
		...
		<div class="container">
			<div
				class="<?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'flex 2xl:gap-[70px] lg:gap-10 gap-8' : '' ?>">
						...
						if ( $response ) :
							...
							<?php if ( wp_is_mobile() && bsc_is_mobile() ) { ?>
								...
								if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) :
									?>
									<ul
										class="overflow-y-auto absolute py-2 z-30 w-full max-h-64 scroll-bar-custom block [&:not(.active)]:opacity-0 opacity-100 [&:not(.active)]:pointer-events-none transition-all duration-500 origin-top-left scale-x-100 [&:not(.active)]:scale-y-0 scale-100 lg:bg-[#F3FBFE] bg-white p-2 prose-a:block rounded text-xs mt-2">
										<?php foreach ( $terms as $term ) :
											$active_class = ( is_tax( 'danh-muc-bao-cao', $term->term_id ) ) ? 'active' : '';
											?>
											<li>
								...
								<?php if ( wp_is_mobile() && bsc_is_mobile() ) { ?>
									<div class="w-[48%]">
										<div
											...
											<label for=""
												class="font-medium text-[12px]"><?php _e( 'Thời gian:', 'bsc' ) ?></label>
