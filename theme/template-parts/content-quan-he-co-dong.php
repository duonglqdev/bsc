<?php if ( $args['data'] ) {
	...
	?>
	<div class="news_service-item document_item-popup flex items-center justify-between  <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? '[&:not(:last-child)]:border-b [&:not(:last-child)]:border-[#E1E1E1] [&:not(:last-child)]:pb-8 lg:gap-20 gap-4' : 'gap-4' ?>"
		data-newstype="0" data-modal-target="document-modal" data-modal-toggle="document-modal" data-doccument="<?php
		if ( $news->attachedfileurl ) {
			echo slug_file_news( htmlspecialchars( $news->newsid ) );
		}
