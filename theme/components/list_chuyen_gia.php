...
<div class="trigger-button" id="open-expert" data-modal-target="expert-modal" data-modal-toggle="expert-modal"></div>
		...
		<div class="relative bg-white rounded-2xl shadow overflow-hidden">
			...
			if ( $has_post ) {
				while ( $query->have_posts() ) {
					...
					$qr_code = get_field( 'ma_qr', $post_id );
					$link = get_field( 'link', $post_id );
					?>

					<div class="mmg-wrapper <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'p-6' : 'p-4' ?>"
						data-mmg="<?= $mmg ?>">
								...
								<ul class="space-y-2 text-xs expert-info">
									...
									if ( $dia_chi ) {
										...
										<li
											...
											<p>
												<?php _e( 'Địa chỉ', 'bsc' ) ?>:
												<strong><?php echo $dia_chi[0]->name; ?></strong>
											</p>
										</li>
									<?php } ?>
								</ul>
