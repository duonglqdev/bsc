...
if ($tax) {
    ...
    if ($groupid) {
        ...
        if ($response) {
?>
            <section class="home_news bg-white kienthuc-news  <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'2xl:my-[100px] my-20':'my-[50px]' ?>" <?php if (get_sub_field('id_class')) { ?> id="<?php echo get_sub_field('id_class') ?>" <?php } ?>>
                <div class="container">
                    <?php if ( !wp_is_mobile() && !bsc_is_mobile()) { ?> 
                        <?php } ?>
                        <?php if( !wp_is_mobile() && !bsc_is_mobile()) : ?>
                            <div class="lg:grid grid-cols-5 gap-5 lg:space-y-0 space-y-5">
                                <?php
                                $i = 0;
