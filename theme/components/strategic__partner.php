<section
	class="strategic__partner <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'lg:py-[105px] py-12' : 'py-[46px]' ?> relative after:absolute after:inset-0 after:w-full after:h-full after:pointer-events-none after:bg-gradient-blue-to-bottom after:opacity-40"
		...
		<div
			...
			<?php if (! wp_is_mobile() && ! bsc_is_mobile()) { ?>
				<div class="col-span-1">
					...
					?>
						<a rel="<?php the_sub_field('url_image_rel') ?>" href="<?php echo check_link(get_sub_field('url_image')) ?>" <?php if (get_sub_field('open_tab')) echo 'target="_blank"' ?>>
							<?php echo wp_get_attachment_image(get_sub_field('img'), 'large','', array('class'=> 'mx-auto')); ?>
						</a>
					<?php
					} else {
						echo wp_get_attachment_image(get_sub_field('img'), 'large','', array('class'=> 'mx-auto'));
					} ?>
				</div>
			<?php } ?>
		</div>
