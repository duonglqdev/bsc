...
?>
<section
	class="<?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'xl:my-[100px] my-20' : 'my-[50px]' ?> qdbsc_du_bao_nganh"
	<?php if (get_sub_field('id_class')) { ?> id="<?php echo get_sub_field('id_class') ?>" <?php } ?>>
	<div class="container">
		<?php if (get_sub_field('title')) { ?>
			<?php if (get_sub_field('link')) { ?>
				<a href="<?php echo check_link(get_sub_field('link')) ?>"
					class="font-bold block <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'mb-6 text-2xl' : 'mb-4 text-[22px]' ?>">
		...
		<div class="relative rounded-[10px] overflow-hidden">
			<div
				...
				?>
					...
					?>
					<!-- Data Demo -->
					<div class="flex text-white bg-primary-300 font-semibold items-center min-h-[34px] leading-[1.125]">
						...
						<?php for ($i = 0; $i < 9; $i++) { ?>
							<div class="flex items-center ">
								<div
									...
									<?php _e('Công nghệ - Viễn thông', 'bsc') ?>
								</div>
								<div
