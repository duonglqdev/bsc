<section class="about_history bg-primary-50 py-14" <?php if ( get_sub_field( 'id_class' ) )
{ ?>
			...
			<div class="about_history-content">
				...
				while ( have_rows( 'history' ) ) :
					...
					$history_items[] = [ 
						...
						'nam' => get_sub_field( 'nam' ),
						'mota' => get_sub_field( 'mota' ),
							...
							{ ?>
								<div class="max-w-[547px] w-full">
									<div class="rounded-[20px] overflow-hidden relative pt-[70.21%] group">
										<?php echo wp_get_attachment_image( $item['img'], 'large', '', array( 'class' => 'absolute w-full h-full inset-0 object-cover transition-all duration-500 group-hover:scale-110' ) ) ?>
									</div>
								</div>
							<?php } ?>
