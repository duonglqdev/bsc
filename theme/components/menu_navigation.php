...
?>
    <section class="bg-primary-50 sticky z-20 <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'2xl:py-4 py-3 top-0':'py-[12px] top-[60px]' ?>" <?php if (get_sub_field('id_class')) { ?> id="<?php echo get_sub_field('id_class') ?>" <?php } ?>>
        ...
        if ($menu_navigation) { ?>
            <div class="container">
                <ul class="flex <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'justify-between gap-10':'overflow-x-auto gap-4  nav-scroll-mb' ?>">
                    <?php foreach ($menu_navigation as $row) { ?>
                        <li class="flex-1 <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'':'whitespace-nowrap' ?>">
                            <a href="<?php echo check_link($row['link']) ?>"
                                class="<?php if ($row['active']) echo 'active' ?> block text-center font-semibold [&:not(.active)]:text-black text-white [&:not(.active)]:bg-transparent bg-primary-300 transition-all duration-500 hover:!text-white hover:!bg-primary-300 rounded-lg <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'text-base py-[12px] px-10':'py-3 px-4 text-xs' ?>">
                                <?php echo $row['title'] ?>
                            </a>
                        </li>
                    <?php } ?>
            ...
            <div class="container">
                <ul class="flex <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'justify-between gap-10':'overflow-x-auto gap-4  nav-scroll-mb' ?>">
                    <?php while (have_rows('menu_navigation')): the_row(); ?>
                        <li class="flex-1 <?php echo !wp_is_mobile() && !bsc_is_mobile() ?'':'whitespace-nowrap' ?>">
                            <a href="<?php echo check_link(get_sub_field('link')) ?>"
                                class="<?php if (get_sub_field('active')) echo 'active' ?> block text-center font-semibold lg:text-base lg:py-[12px] py-3 2xl:px-10 px-5 [&:not(.active)]:text-black text-white [&:not(.active)]:bg-transparent bg-primary-300 transition-all duration-500 hover:!text-white hover:!bg-primary-300 rounded-lg whitespace-nowrap">
                                <?php the_sub_field('title') ?>
                            </a>
                        </li>
                    <?php endwhile; ?>
