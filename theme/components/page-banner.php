...
if ( ( get_sub_field( 'background' ) || get_sub_field( 'background_mobile' ) ) && ( is_single() || is_page() || get_query_var( 'mck_bctc' ) ) ) {
	$banner = wp_get_attachment_image_url(
...
} elseif ( get_sub_field( 'title' ) && ( is_single() || is_page() || get_query_var( 'mck_bctc' ) ) ) {
...
<section <?php if ( get_sub_field( 'id_class' ) ) { ?> id="<?php echo get_sub_field( 'id_class' ) ?>" <?php } ?>
	class="page__banner relative after:absolute after:inset-0 after:w-full after:h-full after:bg-no-repeat after:pointer-events-none <?php echo $style == 'default' && ! wp_is_mobile() && ! bsc_is_mobile() ? 'after:bg-gradient-banner' : 'page__banner-haft' ?> <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'py-24' : 'py-[50px]' ?>  text-white bg-no-repeat bg-cover "
	...
	<div class="container relative z-[1]">
		...
		<?php } ?>
		<h2
			class="font-bold <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'lg:text-[40px] text-4xl' : 'text-2xl' ?>">
			<?php echo $title; ?>
		</h2>
	</div>
</section>