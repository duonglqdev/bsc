...
if ($file) {
    $file_path = str_replace(get_site_url(), ABSPATH, $file); // Chuyển URL thành filesystem path

    // Kiểm tra xem file có tồn tại không
    if (!file_exists($file_path)) {
        die('Tệp không tồn tại hoặc không đọc được: ' . $file_path);
    }
                    ...
                    <div class="map-item relative w-full <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'pt-[76.4424%]' : 'pt-[150%]' ?>" id="map">
                        ...
                        <?php if ( wp_is_mobile() && bsc_is_mobile()) { ?> 
                            ...
                            <div class="w-full rounded-[10px] shadow-base py-2 bg-white">
                                ...
                                <div class="scroll-bar-custom overflow-y-auto max-h-[140px] min-h-[140px] address-list" id="branch-list">
                                </div>
                            </div>
                            </div>     
                        <?php } ?>
