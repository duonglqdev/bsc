...
if ( $file ) {
	...
	if ( ! $file_data ) {
		die( 'Không thể tải nội dung file từ URL.' );
	}

	// Đọc file Excel
	if ( $xlsx = SimpleXLSX::parseData( $file_data ) ) {
		...
		foreach ( $sheet_data as $index => $row ) {
			...
			// Quy định: Cột D = Tên mạng lưới, Cột K = Địa chỉ, Cột J = Tỉnh/Thành, Cột I = Quận/Huyện
			$locations[] = [ 
				'name' => $row[3] ?? '',
				'address' => $row[10] ?? '',
				'city' => $row[9] ?? '',
		...
		$json_locations = ! empty( $locations ) ? json_encode( $locations ) : json_encode( [] );
	} else {
	...
	<section class="<?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'xl:my[100px] my-20' : 'my-[50px]' ?> contact_maps"
		...
		<div class="container">
			...
			<div
				...
				<div class="space-y-2 <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'flex items-center gap-2' : '' ?>">
					...
					<select id="city"
						class="select_custom  py-0 border-[#EAEEF4] h-[38px] max-w-full <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'rounded-[10px] pl-5 w-[160px]' : 'w-full rounded-lg pl-3 text-xs' ?>">
			...
			<div class="<?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'flex gap-12' : '' ?>">
				...
				<div class="flex-1 relative">
					<div class="map-item relative w-full <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'pt-[76.4424%]' : 'pt-[150%]' ?>"
						id="map">
					</div>
					<?php if ( wp_is_mobile() && bsc_is_mobile() ) { ?>
	...
	<script>
		let locations = <?php echo $json_locations; ?>;
		let map;
		let markers = [];
		...
		function updateCityFilter()
		{
			...
			citySelect.addEventListener( 'change', updateBranchList );
		}
		...
		function updateDistrictFilter()
		{
			...
			const uniqueDistricts = [...new Set( filteredLocations.map( loc => loc.district ) )];
			...
			// Sau khi thay đổi tỉnh thành, auto update branch list
			updateBranchList();
		}
		...
		function updateBranchList()
		{
			...
			// Lọc dữ liệu theo tỉnh/thành và quận/huyện
			...
			branchList.innerHTML = ''; // Xóa danh sách cũ
			...
			filteredLocations.forEach( ( loc, index ) =>
			{
				...
				<h3 class="font-bold text-primary-300 mb-2">${ loc.name }</h3>
				<div class="text-xs text-[#4A5568] group-hover:mb-4">${ loc.address }</div>
				<a 
			...
			`;
				...
				// Gắn sự kiện click để cập nhật bản đồ
				branchItem.addEventListener( 'click', () =>
				{
					updateMap( loc.address, branchItem );
				} );
			} );

			// Auto active phần tử đầu tiên sau khi lọc
			if ( filteredLocations.length > 0 )
			{
				const firstBranch = branchList.firstChild;
				updateMap( filteredLocations[0].address, firstBranch );
			}
		}


		// Cập nhật bản đồ khi chọn địa chỉ
		function updateMap( address, activeElement )
		{
			...
			}, ( results, status ) =>
			{
				if ( status === 'OK' )
				{
					...
					// Xóa các lớp active trước đó
					document.querySelectorAll( '.map-address.active' ).forEach( el => el.classList.remove( 'active' ) );
		...
		// Gọi hàm cập nhật danh sách ban đầu
		updateBranchList();
		...
		document.getElementById( 'district' ).addEventListener( 'change', updateBranchList );
