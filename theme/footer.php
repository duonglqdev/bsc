					...
					<?php if ( have_rows( 'f1_thongtintruso', 'option' ) )
					{
						while ( have_rows( 'f1_thongtintruso', 'option' ) ) :
							...
							<li
								...
								<div
									class="<?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? '' : 'w-6' ?> shrink-0">
									<?php
									$icon_field = get_sub_field( 'icon' );
									$icon_values = explode( ',', $icon_field );
									$icon_name = isset( $icon_values[0] ) ? trim( $icon_values[0], " '" ) : '';
				...
				<div class="flex justify-between flex-wrap gap-y-5 -mx-3">
					...
					{ ?>
						<div
							...
							{ ?>
								<p
									class="<?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? '' : 'text-xs' ?>">
	...
	if ( get_field( 'cdc3_link', 'option' ) && get_field( 'cdc3_youtube_open_time', 'option' ) && ! wp_is_mobile() && ! bsc_is_mobile() )
	{
		...
		if ( $cdc3_youtube_open_time_value && $current_date >= $cdc3_youtube_open_time_value )
		{
			...
			<a <?php if ( get_field( 'cdc3_open_tab', 'option' ) )
				...
				href="<?php echo check_link( get_field( 'cdc3_link', 'option' ) ) ?>"
			...
			{ ?>
				<div
					...
					<?php while ( have_rows( 'cdc4_menu', 'option' ) ) :
						the_row(); ?>
						<a href="<?php echo check_link( get_sub_field( 'link' ) ) ?>"
							rel="<?php the_sub_field( 'rel' ) ?>" <?php if ( get_sub_field( 'open_tab' ) )
									echo 'target="_blank"' ?>
							...
							{ ?>
								<div
									class="font-bold <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'text-xs' : 'text-xxs' ?>">
									<?php the_sub_field( 'title' ) ?>
								</div>
							<?php } ?>
						</a>
			...
			<div
				...
				if ( $custom_logo_id )
				{
					...
					<a class="block" href="<?php echo get_bloginfo( 'url' ); ?>"
						...
						<img class="object-contain max-w-[103px] mx-auto"
				...
				<div class="md:mt-[77px] mt-10 text-white ">
					<a href="#" class="transition-all duration-300 hover:scale-105 inline-block">
			...
			<div
				...
				<form action="" class="mt-8">
					<div class="space-y-[12px]">
						<div
							class="user_name border-b border-[#3F4247] h-12 flex items-center gap-3">
						...
						<div
							class="user_password border-b border-[#3F4247] h-12 flex items-center gap-3">
				...
				<div
					class="md:mt-[119px] mt-10 md:grid md:grid-cols-2 md:gap-4 md:space-y-0 space-y-4">
					<a href="#"
