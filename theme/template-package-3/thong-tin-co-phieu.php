...
<main>
	...
	<section
		...
		<div class="container">
			...
			<div
				class="<?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'grid grid-cols-2 gap-5' : 'block_slider block_slider-show-1 fli-dots-blue dots-left' ?>">
				<?php
				for ( $i = 0; $i < 3; $i++ )
				{
					?>
					<div class="rounded-xl overflow-hidden bg-no-repeat bg-cover flex px-8 <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'px-11 py-5 min-h-[228px]' : 'pl-6 py-4 pr-3 block_slider-item w-full min-h-[167px] items-center' ?>"
						...
						<div class="w-1/2 h-full">
							<div
								...
								<p
									class="font-bold mb-2 <?php echo ! wp_is_mobile() && ! bsc_is_mobile() ? 'text-2xl' : 'text-lg' ?>">
									Báo cáo phân tích
								</p>
								<a href="#"
